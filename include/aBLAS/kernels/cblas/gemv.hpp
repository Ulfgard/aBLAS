 ...
 * 
 *
 * \brief      CBLAS bindings to the GEMV routine
 *
 * \author      O. Krause
 * \date        2015
 *
 ...
 *
 */
#ifndef ABLAS_KERNELS_CBLAS_GEMV_HPP
#define ABLAS_KERNELS_CBLAS_GEMV_HPP
...
void gemv(
	matrix_expression<MatrA,cpu_tag> const &A,
	...
	ABLAS_SIZE_CHECK(x().size() == A().size2());
	ABLAS_SIZE_CHECK(y().size() == A().size1());
