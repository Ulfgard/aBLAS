 ...
 * 
 *
 * \brief       Basic functors for assignment operations and elementwise transformations
 * 
 * 
 *
 * \author      O. Krause
 ...
 *
 */
#ifndef ABLAS_DETAIL_FUNCTIONAL_HPP
#define ABLAS_DETAIL_FUNCTIONAL_HPP
...
struct scalar_assign{
	...
	scalar_assign(typename boost::remove_reference<T1>::type alpha):m_alpha(alpha){}
	void operator()(argument1_type t1, argument2_type t2) {
		t1 = m_alpha * static_cast<typename boost::remove_reference<T1>::type const>(t2);
	}
private:
	typename boost::remove_reference<T1>::type m_alpha;
};
template<class T1, class T2>
struct scalar_plus_assign{
	...
	static const bool right_zero_identity = true;
	static const bool left_zero_identity = false;
	scalar_plus_assign(typename boost::remove_reference<T1>::type alpha):m_alpha(alpha){}
	void operator()(argument1_type t1, argument2_type t2) {
		t1 += m_alpha * static_cast<typename boost::remove_reference<T1>::type const>(t2);
	}
private:
...
struct scalar_minus_assign{
	...
	static const bool right_zero_identity = true;
	static const bool left_zero_identity = false;
	scalar_minus_assign(typename boost::remove_reference<T1>::type alpha):m_alpha(alpha){}
	void operator()(argument1_type t1, argument2_type t2) {
		t1 -= m_alpha * static_cast<typename boost::remove_reference<T1>::type const>(t2);
	}
private:
...
struct scalar_multiply_assign{
	...
	scalar_multiply_assign(typename boost::remove_reference<T1>::type alpha):m_alpha(alpha){}
	void operator()(argument1_type t1, argument2_type t2) {
		t1 *= m_alpha * static_cast<typename boost::remove_reference<T1>::type const>(t2);
	}
private:
...
struct scalar_divide_assign{
	...
	static const bool right_zero_identity = false;
	static const bool left_zero_identity = true;
	scalar_divide_assign(typename boost::remove_reference<T1>::type alpha):m_alpha(alpha){}
	void operator()(argument1_type t1, argument2_type t2) {
		t1 /= m_alpha * static_cast<typename boost::remove_reference<T1>::type const>(t2);
	}
private:
