 ...
 * 
 *
 * \brief       Basic iterators and iterator handling
 *
 * \author      O. Krause
 * \date        2015
 *
...
#ifndef ABLAS_DETAIL_ITERATOR_HPP
#define ABLAS_DETAIL_ITERATOR_HPP
...
class dense_storage_iterator:
	public random_access_iterator_base<
		dense_storage_iterator<RandomAccessIterator>,
...
public:
	...
	dense_storage_iterator(RandomAccessIterator iterator, size_type index, size_type stride=1)
	: m_index(index), m_stride(stride), m_iterator(iterator) {}
		
	template<class C>
	dense_storage_iterator(dense_storage_iterator<C> const& iterator)
	: m_index(iterator.m_index), m_stride(iterator.m_stride), m_iterator(iterator.m_iterator) {}
	
		
	// Assignment
	template<class T>
	dense_storage_iterator &operator = (dense_storage_iterator<T> const& it) {
	...
	template<class T>
	difference_type operator - (dense_storage_iterator<T> const& it) const {
		return m_index - it.m_index;
	}

	// Dereference
	reference operator *() const {
		return m_iterator[m_index*m_stride];
	}
	reference operator [](difference_type n) const {
		return m_iterator[(m_index+n)*m_stride];
	}
	...
	// Comparison
	template<class T>
	bool operator == (dense_storage_iterator<T> const& it) const {
		return m_index == it.m_index;
	}
	template<class T>
	bool operator < (dense_storage_iterator<T> const& it) const {
...
public:
	...
	compressed_storage_iterator &operator -- () {
		ABLAS_RANGE_CHECK(m_position > 0);
		--m_position;
		return *this;
	}
	...
	difference_type operator - (compressed_storage_iterator<U,V> const& it) const {
		ABLAS_RANGE_CHECK(m_values == it.m_values);
		ABLAS_RANGE_CHECK(m_indices == it.m_indices);
...
private:
	...
	Iterator incrementToIndex(
		...
		ABLAS_SIZE_CHECK(iter.index()<= index);
	...
	Iterator incrementToIndex(
		...
		if(iter < end ){
			ABLAS_RANGE_CHECK(iter.index() < index);
